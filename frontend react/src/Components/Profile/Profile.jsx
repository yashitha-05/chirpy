...
import { Avatar, Backdrop, Box, Button, CircularProgress, Modal, IconButton } from "@mui/material";
    ...
    <React.Fragment>
      ...
      <section className="pl-6">
        <div className=" flex justify-between items-start mt-5 h-[5rem]">
          ...
          />
          {auth.findUser?.req_user || auth.user?.id === auth.findUser?.id ? (
            <Button
        ...
        <div>
          ...
          <div className="mt-2 space-y-3">
            ...
            <div className="flex items-center space-x-5">
              <div 
                className="flex items-center space-x-1 font-semibold cursor-pointer hover:underline"
      ...
      >
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <IconButton onClick={() => setOpenFollowersModal(false)}>
              <CloseIcon />
            </IconButton>
          </div>
          <div className="space-y-3">
            {auth.findUser?.followers?.length > 0 ? (
              auth.findUser.followers.map((follower) => (
                <div
                  key={follower.id}
                ...
                >
                  <div className="flex items-center space-x-3">
                    <Avatar src={follower.image} alt={follower.fullName} />
                    <div>
              ...
              <p className="text-center text-gray-500 py-5">No followers yet</p>
            )}
          </div>
        </Box>
      </Modal>
      ...
      >
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <IconButton onClick={() => setOpenFollowingModal(false)}>
              <CloseIcon />
            </IconButton>
          </div>
          <div className="space-y-3">
            {auth.findUser?.followings?.length > 0 ? (
              auth.findUser.followings.map((following) => (
                <div
                  key={following.id}
                  className="flex items-center justify-between p-3 hover:bg-slate-800 rounded cursor-pointer"
                  onClick={() => {
                ...
                >
                  <div className="flex items-center space-x-3">
                    <Avatar src={following.image} alt={following.fullName} />
                    <div>
              ...
              <p className="text-center text-gray-500 py-5">Not following anyone yet</p>
            )}
          </div>
        </Box>
      </Modal>
